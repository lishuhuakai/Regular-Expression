	...
	notIncludeContent_(false) /* notIncludeContent_一般用于[^x],如果其为true,表示不包含x */
{
	...
	else if (pattern.size() != 0 && pattern[0] == L'[') { 
		/* 现在开始解析[], []表达式包含这些形式:
		 * [abc][a-c][^abc][^a-c] 
		 */
		parseRange(pattern);
	...
	greedy_(true)
{
	 ...
	 * *, +, {m,}, {,n}, {m,n}
	 * 还有一个可选操作,那就是?,在重复之后,表示非贪婪
	 */
	wstring pt = pattern;
	size_t size = pattern.size();
	if (size >= 2 && pt[size - 1] == L'?') {
		greedy_ = false;
		pt = pt.substr(0, size - 1); /* 去除尾部的'?' */
		size = pt.size();
	}
	...
	if (size == 1) {
		...
		else if (pt == L"+") { /* 匹配一次或者多次 */
			minTimes_ = 1; maxTimes_ = -1; infinity_ = true;
		}
		else
			assert(0);
	}
	else if (pt[0] == L'{') { /* 可以保证,有 {, 那么必定有 } */
		getRepeateTimes(pt.substr(1, size - 2)); /* 去掉开头的"{"以及结尾的"}" */
	}
	else
		assert(0);
}

MethodNode::MethodNode(wstring& pattern):
	Node(method, pattern)
{
	methodtype_ = MethodNode::Check;
	assert(pattern.size() > 7);
	name_ = pattern_.substr(5, pattern_.size() - 7);
}

MethodNode::MethodNode(wstring& pattern, nodePtr& sub) :
	Node(method, pattern, sub)
{
	// 接下来开始解析方法,方法节点都是从(?开始的,因此可以省略前面的两个符号
	// pattern一共有这么一些形式,如
	// (? 匿名捕获
	// (?:匿名捕获
	// (?:<#xxx> 命名捕获
	...
	// (?!反向预查
	size_t size = pattern.size();
	if ((pattern[0] == L'?') && (size == 1))
		methodtype_ = NonGreedy;
	else {
		assert(pattern[0] == L'(' && pattern[1] == L'?');
		if (size == 2)
			methodtype_ = anonymousCatch;
		else if ((pattern[2] == L'=') && (size == 3))
			methodtype_ = forwardMatch;
		else if ((pattern[2] == L'!') && (size == 3))
			methodtype_ = backwardMatch;
		else if ((pattern[2] == L':') && (size == 3))
			methodtype_ = anonymousCatch;
		else if ((pattern[2] == L':') && (pattern[3] == L'<') && (size > 6)) {
			assert(pattern[pattern.size() - 1] == L'>');
			if (pattern[4] == L'#')
				methodtype_ = Catch;
			else if (pattern[4] == L'$')
				methodtype_ = Check;
			name_ = pattern_.substr(5, pattern_.size() - 6);
		}
		else
			assert(0);
	}
