 ...
 */
tokenPtr Lex::productToken(Token::tokenType t, int len, int operators) { 
	...
	while (currentPos_ != expression_.end()) {
		if (L'\\' == *currentPos_) { /* 如果是转义字符 */
			...
			case L'*': {
				int len = 2;
				++currentPos_;
				if (currentPos_ == expression_.end() || *currentPos_ != L'?') len = 1;
				--currentPos_;
				return productToken(Token::kRepeate, len, 1); /* ?代表贪心 */
			}
			default:
					...
					if (*nextPos == L'<') { // 这里包含两种形式,一种是(?:<#xxx> 命名捕获 以及 (?:<$xxx>命名检查,不再细分下去
						...
						if (nextPos != expression_.end()) {
							if (*nextPos == L'$') {
								...
								return productToken(Token::kCheck, nextPos - currentPos_ + 1);
							}
						}
						nextPos = find(nextPos, expression_.end(), L'>');
...
/*
 * handleSquareBracket 主要用于处理[xyz]以及[^xyz]符号,
 * [xyz]一般表示从xyz中选择一个,而[^xyz]一般表示从xyz之外的元素选择一个
 */
tokenPtr Lex::handleSquareBracket() { 
